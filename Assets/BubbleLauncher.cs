...
public class BubbleLauncher : MonoBehaviour
{
    ...
    [Header("Skills")]
    // Skill hook: while true, the next launched bubble is a wildcard that adopts
    // the color of whatever bubble it touches first. Consumed on launch.
    ...
    private static readonly BubbleColor[] AllColors =
        (BubbleColor[])System.Enum.GetValues(typeof(BubbleColor));

    private JellyCore core;
    ...
    void Awake()
    {
        ...
        core = FindFirstObjectByType<JellyCore>();
        InitializeDotPool();
    }
    ...
    private void InitializeDotPool()
    {
        ...
        if (dotPrefab == null)
        {
            Debug.LogWarning("BubbleLauncher: no dot prefab assigned for the aim preview.");
            return;
        }
    ...
    private void LaunchBubble()
    {
        ...
        {
            Debug.LogWarning("BubbleLauncher: no bubble prefab assigned.");
            return;
        }
        ...
        GameObject bubbleObject = Instantiate(bubblePrefab, spawnPosition, Quaternion.identity);

        if (!bubbleObject.TryGetComponent(out Bubble bubble))
        {
            bubble = bubbleObject.AddComponent<Bubble>();
        }
        ...
        if (isRainbowActive)
        {
            // Skill hook: mark the spawned bubble as a wildcard here once
            // Bubble/BubbleMerge grow rainbow support (adopt color on first touch).
            isRainbowActive = false;
        }

        if (bubbleObject.TryGetComponent(out Rigidbody2D rb))
        {
        ...
        {
            Debug.LogWarning("BubbleLauncher: bubble prefab has no Rigidbody2D.");
        }
    ...
    // per-physics-step integration the real bubble will experience — pulling
    // JellyCore.GetPullForce each step (bubble mass is 1, so force equals
    ...
    private void DrawTrajectory()
    {
        ...
        // slow bubble's full-length deep curve.
        ...
        for (int i = 0; i < activeSteps; i++)
        {
            if (core != null)
            {
                velocity += core.GetPullForce(position) * dt;
                // Same orbit braking the real bubble gets in JellyCore's
                // FixedUpdate — without this the dots would overshoot the
                // curve on off-center shots.
                velocity = core.ApplyOrbitBrake(position, velocity, dt);
            }
        ...
        Color liveColor = BubbleColorPalette.ToUnityColor(CurrentColor);
