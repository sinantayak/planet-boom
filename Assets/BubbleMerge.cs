...
{
    // Two bubbles at this level don't level up — they trigger the BOOM chain
    // explosion instead (both die + every same-color bubble in the scene).
    [SerializeField] private int maxLevel = 3;

    // Fractional size gain per tier, applied to the bubble's OWN spawn scale:
    ...
    // The tier-1 scale this bubble spawned with, captured before any merging.
    ...
    // Winner side: currently pulling a losing bubble into itself.
    private bool isAbsorbing;

    // Loser side: currently being pulled into a winner; physics is off and this
    // bubble must be ignored by every other merge check until it is destroyed.
    ...
    void Start()
    {
        ...
        // fall asleep, which happens right as two bubbles drift to a near-zero
    ...
    void FixedUpdate()
    {
        ...
        // across steps, or a bubble that already won one merge could never merge
        // again afterwards.
        hasMerged = false;
    ...
    // Bypasses Unity's collision events entirely: any same-color bubble whose
    ...
    private void CheckProximityMerge()
    {
        ...
        foreach (Bubble otherBubble in FindObjectsByType<Bubble>(FindObjectsSortMode.None))
        {
            if (otherBubble == bubble || !otherBubble.gameObject.activeInHierarchy)
                continue;

            if (otherBubble.CurrentColor != bubble.CurrentColor ||
                otherBubble.Level != bubble.Level)
                continue;

            if (!otherBubble.TryGetComponent(out CircleCollider2D otherCollider))
                continue;

            float otherRadius = otherCollider.radius * otherBubble.transform.lossyScale.x;
            float touchDistance = (myRadius + otherRadius) * touchDistanceMultiplier;
            float distance = Vector2.Distance(transform.position, otherBubble.transform.position);

            if (distance <= touchDistance && TryMergeWith(otherBubble.gameObject))
                break;
        }
    }
    ...
    private bool TryMergeWith(GameObject other)
    {
        ...
        // A bubble already melting into someone (or busy pulling one in) is spoken
        // for — starting a second fusion with it would orphan or double-count it.
        if (!other.TryGetComponent(out BubbleMerge otherMerge) ||
            otherMerge.IsBeingAbsorbed || otherMerge.isAbsorbing)
            return false;

        // Only the bubble with the lower UniqueId performs the merge, so a single
        // contact between two matching bubbles doesn't get processed from both
        // sides (which would destroy both instead of merging them into one).
        if (bubble.UniqueId > otherBubble.UniqueId)
            return false;

        hasMerged = true;

        // Two max-level bubbles don't produce a bigger bubble — they detonate.
        if (bubble.Level >= maxLevel)
        {
    ...
    // BOOM: both max-level bubbles die, and the blast chains to every other
    // active bubble of the same color anywhere in the scene. Everything caught
    // in the blast is flagged absorbed *before* the deferred Destroy lands, so
    // no other bubble's collision/proximity check can start a fusion with a
    // corpse during the remainder of this physics step.
    private void TriggerBoom(BubbleMerge partner)
    {
        BubbleColor boomColor = bubble.CurrentColor;
        ...
        foreach (Bubble victim in FindObjectsByType<Bubble>(FindObjectsSortMode.None))
        {
            // Same color only — the two trigger bubbles are swept up by this
            ...
            if (victim.TryGetComponent(out BubbleMerge victimMerge))
            {
    ...
    // External hook (GameManager board clears): takes this bubble out of the
    // merge system ahead of a deferred Destroy, the same guarantee TriggerBoom
    // gives its victims — no other bubble can start a fusion with a corpse.
    public void PrepareForDespawn()
    {
        BeginBeingAbsorbed();
    }

    // Loser side of the fusion: freeze physics right away so the melting bubble
    ...
    // larger circle.
    private IEnumerator FuseWith(BubbleMerge loser)
    {
        float startScale = transform.localScale.x;
        float targetScale = ScaleForLevel(bubble.Level);
